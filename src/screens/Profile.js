...
import { View, Text, StyleSheet, Dimensions, ScrollView, TouchableOpacity, Image, TextInput, Keyboard, PermissionsAndroid, Alert } from 'react-native'
import AntDesign from 'react-native-vector-icons/AntDesign';
import { useDispatch, useSelector } from 'react-redux'
import { logoutUser, updateUserData } from '../store/actions/LoginActions';
...
export default function Profile({ route, navigation }) {
  ...
  const profile_status = useSelector(state => state.LoginDetails.profile_status);
  const user_data = useSelector(state => state.LoginDetails);
  const update_login_details = useDispatch()
  ...
  const SaveProfile = async () => {
    ...
    if (jsonPre.success) {
      ...
      if (json != null && json.length > 0 && json[0].response != null && json[0].response != "error") {
        ...
        update_login_details(updateUserData(user_data))
        navigation.goBack()
        alert("Profile Saved!!")
      }
    }
  ...
  useEffect(() => {
    if (user_data.lat != 0 && user_data.lng != 0 && areas.length > 0) {
